...
export default async function BlogPostPage({ params }: { params: { slug: string } }) {
  ...
  // dateModified removed: there's no real last-modified field on posts, so
  // this was previously set equal to datePublished on every render — which
  // silently goes stale (or wrong) the moment a post is actually edited
  // without that edit being tracked anywhere. Same category of bug already
  // fixed on the Springfield MO plumber site (dateModified hardcoded to
  // "today" there instead). Omitting the field entirely is honest; add it
  // back only once posts have a real lastModified value to source it from.
  const articleSchema = { "@context": "https://schema.org", "@type": "Article", headline: post.title, description: post.metaDesc, datePublished: post.date, author: { "@type": "Organization", name: "Clarksville Water Damage Restoration", url: SITE_URL }, publisher: { "@type": "Organization", name: "Clarksville Water Damage Restoration", url: SITE_URL }, mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/blog/${params.slug}` }, articleSection: post.category };
